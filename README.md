...
# open a plot window with 1 row and 2 columns

par(mfrow=c(2,3))

hist(y.p,main="Frequency Histogram")
boxplot(y.p,main="Boxplot",
  ylab="Distribution of y.p")
...
# look at histograms/boxplot showing the sampling 
# distribution of the mean and the median.

hist(y.s.mean,main="Sampling Distribution of the Sample Mean")
hist(y.s.median,main="Sampling Distribution of the Sample Median")

boxplot(y.s.mean,y.s.median,xaxt="n",ylab="Sampling Distribution")
...
<p align="center">
<img src="/gfiles/fig3c.png" width="800px">
</p>
...
To motivate our discussion, let's suppose we have a well-defined population of prison releasees; some of the prison releasees went through a drug treatment program while in prison while others did not. Each of the prison releasees is followed up for 3 years to measure recidivism rates for the two groups. Here is the population:

```R
x <- c(rep("treatment",500000),rep("control",500000))
y <- c(rep("yes",350000),rep("no",150000),rep("yes",350000),rep("no",150000))
ptable <- table(y,x)
ptable
pfail.treated <- ptable[2,1]/(ptable[1,1]+ptable[2,1])
pfail.treated
pfail.control <- ptable[2,2]/(ptable[1,2]+ptable[2,2])
pfail.control
delta <- pfail.treated-pfail.control
delta
```
and here is the output:
...
Notice that this implies a 70% failure rate for both groups. So, we have now established that the value of delta = p(fail|treated) - p(fail|control) = 0 in the population. Next, let's draw a random sample from this population:

```R
set.seed(38227)
popdf <- data.frame(x,y)
sampsize <- 1000
sdf <- popdf[sample(nrow(popdf),size=sampsize,replace=T), ]
head(sdf,n=10)
stable <- table(sdf$y,sdf$x)
stable
sample.pfail.treated <- stable[2,1]/(stable[1,1]+stable[2,1])
sample.pfail.treated
sample.pfail.control <- stable[2,2]/(stable[1,2]+stable[2,2])
sample.pfail.control
sample.delta <- sample.pfail.treated-sample.pfail.control
sample.delta
```
...
> head(sdf,n=10)
               x   y
130410 treatment yes
181623 treatment yes
479795 treatment  no
365536 treatment  no
293142 treatment yes
948212   control  no
360674 treatment  no
808153   control yes
75012  treatment yes
...
So, the delta in our sample is negative -- p(fail|treated) - p(fail|control) = -0.029. We need a way to judge whether this difference is statistically significant using a conventional 95% confidence level. Our first task is to clearly say what the null hypothesis is: the population value of delta is equal to exactly zero. We now want to discern whether the evidence in our single sample is strong enough to reject this hypothesis. In other words, how surprised will we be to see a difference as large as -0.029 in a single random sample if the true population value is equal to zero? The exercise of measuring how surprised we would be is called a significance test. If p(obtain a delta at least as large as -0.029 | population delta = 0) is a small number then we are "very surprised." A valid significance test for this kind of problem is a Pearson's chi-square test of independence:

```R
# identify observed cell frequencies
...
stotal <- o11+o21+o12+o22
stotal

# calculate expected cell frequencies

e11 <- c1total*r1total/stotal
e21 <- c1total*r2total/stotal
e12 <- c2total*r1total/stotal
e22 <- c2total*r2total/stotal

# calculate chi-square statistics for each cell
...
cstest <- cs11+cs21+cs12+cs22
cstest
critical.value <- qchisq(.95, df=1)
critical.value
pvalue <- 1-pchisq(q=cstest,df=1)
pvalue

# Verify result

chisq.test(stable,correct=F)
```
...
data:  stable
X-squared = 1.0518, df = 1, p-value = 0.3051

> 
```

We can also conduct a likelihood ratio chi-square test which is normally only slightly different than the Pearson test:

```R
stable <- table(sdf$y,sdf$x)
stable

pfail.constrained <- (stable[2,1]+stable[2,2])/sum(stable)
pfail.constrained
pfail.control <- stable[2,1]/(stable[1,1]+stable[2,1])
pfail.control
pfail.treated <- stable[2,2]/(stable[1,2]+stable[2,2])
pfail.treated

loglike.constrained <- stable[1,1]*log(1-pfail.constrained)+
                       stable[2,1]*log(pfail.constrained)+
		       stable[1,2]*log(1-pfail.constrained)+
		       stable[2,2]*log(pfail.constrained)
loglike.constrained
		       
loglike.free <-        stable[1,1]*log(1-pfail.control)+
                       stable[2,1]*log(pfail.control)+
		       stable[1,2]*log(1-pfail.treated)+
		       stable[2,2]*log(pfail.treated)    
loglike.free

lrtest <- 2*(loglike.free-loglike.constrained)
lrtest
pvalue <- 1-pchisq(q=lrtest,df=1)
pvalue
```
